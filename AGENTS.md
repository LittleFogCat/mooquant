...
- **`qmt_server.py`** — 主进程通过 `spawn` 启动的 stdio JSON-RPC 服务（行分隔 JSON）。请求 `{"id","method","params"}`，响应 `{"id","result"}` 或 `{"id","error"}`。
  - 已实现方法：`ping`（心跳）、`quote.*`（行情快照/K线/订阅）、`trade.*`（下单/撤单/持仓/资金）、`stock.*`（股票同步/列表）、`strategy.list/signal/export/add/delete/get_code`（策略框架：列策略/算信号/导出/添加/删除/获取源码）
...
- Git采用Conventional Commit规范，提交信息使用中文

### 版本号规范

- 版本号取文件 `config.js` 中的 `version` 字段
- 版本号格式为：`major.minor.patch`
- 每次进行修改之后，更新 `patch` 版本号
- `major` 与 `minor` 版本号仅在用户显示要求时才更新，禁止任意修改
