---
name: quantdemo
description: AI 量化交易演示项目。提供通达信日线数据下载与转换、QMT 行情数据获取、特征工程、机器学习选股、回测验证、实盘交易。
---
...
```bash
# xtquant 安装与升级
python scripts/install_xtquant.py                              # 自动下载最新版并安装
python scripts/install_xtquant.py --status                     # 查看当前状态
...
python scripts/ai_strategy.py --live                           # 实盘模式（需 QMT + 已训练模型）
...
xtquant 不是 PyPI 包，需从迅投官方下载 RAR 后解压安装。详见 `python scripts/install_xtquant.py`。
...
QMT 客户端 (XtItClient.exe)
  ...
  → signals.py → buy/hold/sell 信号
  → selector.py → Top-N 选股
  → backtest.py（回测验证）/ trader.py + risk.py（实盘交易）
```
...
- **Python** 使用系统 Python `D:\software\python\python.exe`
